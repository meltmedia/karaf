...
public class BootstrapLogManager {
    ...
    private static KarafProperties karafProperties;
    ...
    public static synchronized Handler getDefaultHandler () {
        ...
        filename = karafProperties.getBootstrapLog();
        ...
        if (filename != null) {
            ...
            try {
                fis = new FileInputStream(new File(karafProperties.getBase() + "./etc/org.ops4j.pax.logging.cfg"));
                props.load(fis);
            } catch (IOException e) {
    ...
    public static void setProperties(KarafProperties karafProperties) {
        BootstrapLogManager.karafProperties = karafProperties;
    }
